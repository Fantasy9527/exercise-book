define(["memberTypeApp"],function(app){
...
app.config(function ($stateProvider, $urlRouterProvider) {
  $urlRouterProvider.otherwise('/chargesAccount');
  $stateProvider
    .state('/chargesAccount', {
      url: '/chargesAccount',
      templateUrl: '/assets/templates/memberapp/memberChargesAccount.html',
      controller: 'chargesAccountController'
    })
    .state('/consumesAccount', {
      url: '/consumesAccount',
      templateUrl: '/assets/templates/memberapp/memberConsumesAccount.html',
      controller: 'consumesAccountController'
    })
    .state('/checkBalance', {
      url: '/checkBalance',
      templateUrl: '/assets/templates/memberapp/checkBalance.html',
      controller: 'checkBalanceController'
    })
});


app.controller('chargesAccountController', ["$scope", "$http", "$location", "$document",
    ...
    function getBase() {
      ...
      $http.get("/home/brand/" + brandId + "/member/charges/base", {
        params: params
    ...
    function getData() {
      $location.search(params);
      $http.get("/home/brand/" + brandId + "/member/charges", {
        params: params
      }).success(function (res) {
        res.memberCharges.forEach(function (io, index) {
          io.No = index + 1 + (res.pages.current * 10)
        })
        $scope.memberCharges = res.memberCharges;
...
app.controller('consumesAccountController', ["$scope", "$http", "$location", "$document",
    function ($scope, $http, $location, $document) {
    ...
    function getBase() {
      ...
      $http.get("/home/brand/" + brandId + "/member/consumes/base", {
        params: params
      }).success(function (res) {
        //console.log(res);
        if (!res.saleStaffs) {
    ...
    function getData() {
      $location.search(params);
      $http.get("/home/brand/" + brandId + "/member/consumes", {
        params: params
      }).success(function (res) {
        res.memberConsumes.forEach(function (io, index) {
          io.No = index + 1 + (res.pages.current * 10)
        })
        $scope.memberConsumes = res.memberConsumes;
