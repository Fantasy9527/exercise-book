define(["systemApp"], function(app) {
  app.controller("discount.scopeController", ["$scope", "$http", "$location", "$document", "$rootScope","$state", function($scope, $http, $location, $document, $rootScope,$state) {
    $scope.isLoding = true;
    ...
    $http.get("/home/brand/" + brandId + "/area/country/1/shops").success(function(res) {
      $http.get("/home/brand/" + brandId + "/shop/has/discount/plan/" + $state.params.discount).success(function(disShops) {
    ...
    $scope.save = function(obj, e) {
      ...
      var json = {
        ids: [obj.id], //门店id
        pId: $state.params.discount, // 折扣id,
        state: $scope.state
      };
    ...
    $scope.cancel = function(obj, e) {
      e.stopPropagation();
      $http.delete("/home/brand/" + brandId + "/shop/" + obj.id + "/has/discount/" + $state.params.discount + "/state/" + $scope.state).success(function(res) {
        obj.isDiscount = 1;
        $scope.states[1].length--;
        $scope.states[2].length++;
